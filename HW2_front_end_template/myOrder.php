...
if($_SESSION['Authenticated']==false)
  {
	...
	EOT;
			...
			if($sType == 3)
			{
				$stmt = $conn->prepare("select oid, state, start_time, end_time,sid, price from ord where uid = ?");
				$stmt->bind_param("i", $_SESSION['uid']);
			}
			else
			{
				$stmt = $conn->prepare("select oid, state, start_time, end_time,sid, price from ord where uid = ? and state = ?");
				$stmt->bind_param("ii", $_SESSION['uid'], $sType);
			}
			$stmt->execute();
			...
			while($row = $result -> fetch_assoc())
			{
				...
				echo <<< EOT
					...
					<td>{$shop}</td>
					<td>{$price}</td>
					<form action = myOrderDetail.php method = "post">
					<input type = "hidden" name = "sType" id = "sType" value = {$_POST['sType']}>
					<input type="hidden" name="oid" id="oid" value={$oid}>
					<td><input type="submit" class="btn btn-info" value="order details"></td>
					</form>
				EOT;	
				if($status == "Not Finished")
					echo <<< EOT
						<form action = myOrderCancel.php method = "post">
						<input type="hidden" name="oid" id="oid" value={$oid}>
